...
public class LifeEnchantment {
    ...
    private static void initializeEnchantments() {
        for (Enchantment enchantment : Enchantment.values()) {
            ...
            if (key.equals(MANA_BOOSTER_KEY)) {
                MANA_BOOSTER = enchantment;
                ENCHANTMENT_MAP.put(
                        MANA_BOOSTER,
                        List.of(
                                "§f- §fレベルごとに最大マナ量を",
                                "  §d" + ManaUtil.getManaBoostPlaceholder() + "§f増加させる。"
                        ));
                ...
                ENCHANTMENT_MAP.put(
                        ...
                        List.of(
                                "§f- §fレベルごとに時間経過回復の",
                                "  §fマナを§d" + ManaUtil.getManaRegenPlaceholder() + "§f増加させる。"
                        ));
                ...
                ENCHANTMENT_MAP.put(
                        ...
                        List.of(
                                "§f- §fMob討伐時に10%で発動。",
                                "  §fレベルごとに§d" + ManaUtil.getManaStealPlaceholder() + "§fマナを回復する。"
                        ));
    ...
    public static List<Enchantment> getCustomEnchantments() {
        return new ArrayList<>(List.of(MANA_BOOSTER, MANA_REGEN, MANA_STEAL, ALL_ELEMENT_DAMAGE, ALL_DAMAGE, ALL_DEFENCE));
    }
